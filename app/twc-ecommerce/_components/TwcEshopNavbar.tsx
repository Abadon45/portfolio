...
export default function TwcEshopNavbar({ count, mode, onCart, onToggleMode }: { count: number; mode: "light" | "dark"; onCart: () => void; onToggleMode: () => void }) {
  ...
  const goToSection = (section: "collections" | "journal" | "about" | "story" | "benefits" | "faq") => {
    setMenuOpen(false);
  ...
  const headerProps = { count, mode, onCart, onToggleMode, themeName, setThemeName, menuOpen, setMenuOpen, query, setQuery, submitSearch, go, goToSection };
  ...
  if (themeConfig.headerVariant === "wellness") return <WellnessNavbar {...headerProps} />;
...
type NavbarProps = {
  ...
  submitSearch: () => void;
  go: (path: string) => void;
  goToSection: (section: "collections" | "journal" | "about" | "story" | "benefits" | "faq") => void;
};
...
function EditorialNavbar(props: NavbarProps) {
  return <><AppBar position="sticky" elevation={0} sx={{ bgcolor: "background.paper", color: "text.primary", borderBottom: 1, borderColor: "divider" }}><Container maxWidth="lg"><Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between", minHeight: { xs: 68, md: 82 } }}><IconButton onClick={() => props.setMenuOpen(true)} sx={{ display: { xs: "inline-flex", md: "none" } }} aria-label="Open editorial menu"><MenuRoundedIcon /></IconButton><HeaderLogo centered onClick={() => props.go("/twc-ecommerce")} /><Stack direction="row" spacing={1} sx={{ alignItems: "center", display: { xs: "none", md: "flex" } }}><Button onClick={() => props.go("/twc-ecommerce/shop")} sx={{ color: "text.primary", fontSize: 12 }}>Shop</Button><Button onClick={() => props.goToSection("collections")} sx={{ color: "text.primary", fontSize: 12 }}>Collections</Button><Button onClick={() => props.goToSection("journal")} sx={{ color: "text.primary", fontSize: 12 }}>Journal</Button></Stack><Stack direction="row" sx={{ alignItems: "center" }}><IconButton onClick={() => props.go("/twc-ecommerce/shop")} aria-label="Search products" sx={{ display: { xs: "none", sm: "inline-flex" } }}><SearchRoundedIcon /></IconButton><IconButton onClick={() => props.goToSection("about")} aria-label="About this storefront"><InfoOutlinedIcon /></IconButton><ThemeSelect {...props} /><ThemeModeAndCart {...props} /></Stack></Stack></Container></AppBar><PremiumDrawer {...props} /></>;
}

function WellnessNavbar(props: NavbarProps) {
  return <><AppBar position="sticky" elevation={0} sx={{ bgcolor: "background.paper", color: "text.primary", borderBottom: 1, borderColor: "divider" }}><Container maxWidth="xl" disableGutters sx={{ px: { xs: 2, md: 4 } }}><Toolbar disableGutters sx={{ gap: { xs: 1, md: 2 }, minHeight: { xs: 68, md: 76 } }}><IconButton onClick={() => props.setMenuOpen(true)} sx={{ display: { xs: "inline-flex", lg: "none" }, color: "primary.main" }} aria-label="Open wellness menu"><MenuRoundedIcon /></IconButton><HeaderLogo onClick={() => props.go("/twc-ecommerce")} /><Stack direction="row" spacing={1} sx={{ display: { xs: "none", lg: "flex" }, ml: 2 }}><Button onClick={() => props.go("/twc-ecommerce/shop")} sx={{ color: "text.primary" }}>Shop</Button><Button onClick={() => props.goToSection("collections")} sx={{ color: "text.primary" }}>Collections</Button><Button onClick={() => props.goToSection("story")} sx={{ color: "text.primary" }}>Our story</Button><Button onClick={() => props.goToSection("faq")} sx={{ color: "text.primary" }}>FAQ</Button></Stack><Box sx={{ flexGrow: 1 }} /><TextField autoComplete="off" value={props.query} onChange={(event) => props.setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") props.submitSearch(); }} placeholder="Search wellness essentials" size="small" sx={{ display: { xs: "none", md: "flex" }, maxWidth: 300 }} slotProps={{ input: { startAdornment: <InputAdornment position="start"><SearchRoundedIcon fontSize="small" /></InputAdornment> } }} /><Box sx={{ display: { xs: "none", lg: "block" } }}><ThemeSelect {...props} /></Box><ThemeModeAndCart {...props} /></Toolbar></Container></AppBar><WellnessDrawer {...props} /></>;
}

function WellnessDrawer(props: NavbarProps) {
  const theme = storefrontThemes[props.themeName];
  const dark = props.mode === "dark";
  return <Drawer anchor="left" onClose={() => props.setMenuOpen(false)} open={props.menuOpen}><Box sx={{ bgcolor: dark ? theme.darkPaper : theme.lightSurface, color: dark ? theme.darkText : "#173b27", minHeight: "100%", p: { xs: 2.5, sm: 3 }, width: { xs: 300, sm: 340 } }}><Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between" }}><HeaderLogo onClick={() => props.go("/twc-ecommerce")} /><IconButton onClick={() => props.setMenuOpen(false)} aria-label="Close wellness menu"><CloseRoundedIcon /></IconButton></Stack><Box sx={{ bgcolor: dark ? theme.darkSurface : "#e4f0e4", mt: 4, p: 2.25 }}><Typography sx={{ color: dark ? theme.accent : "#347153", fontSize: 11, fontWeight: 900, letterSpacing: ".14em" }}>WELLNESS ORGANIC</Typography><Typography sx={{ fontSize: 18, fontWeight: 800, lineHeight: 1.25, mt: .75 }}>Good choices for real routines.</Typography></Box><Stack spacing={.75} sx={{ mt: 3 }}><Button onClick={() => props.go("/twc-ecommerce")} sx={{ color: "inherit", justifyContent: "flex-start", fontWeight: 800, px: 1.5 }}>Home</Button><Button onClick={() => props.go("/twc-ecommerce/shop")} sx={{ color: "inherit", justifyContent: "flex-start", fontWeight: 800, px: 1.5 }}>Shop all</Button><Button onClick={() => props.goToSection("collections")} sx={{ color: "inherit", justifyContent: "flex-start", fontWeight: 800, px: 1.5 }}>Collections</Button><Button onClick={() => props.goToSection("story")} sx={{ color: "inherit", justifyContent: "flex-start", fontWeight: 800, px: 1.5 }}>Our story</Button><Button onClick={() => props.goToSection("benefits")} sx={{ color: "inherit", justifyContent: "flex-start", fontWeight: 800, px: 1.5 }}>Why shop with us</Button><Button onClick={() => props.goToSection("faq")} sx={{ color: "inherit", justifyContent: "flex-start", fontWeight: 800, px: 1.5 }}>FAQ</Button></Stack><Divider sx={{ my: 3 }} /><TextField fullWidth label="Search products" value={props.query} onChange={(event) => props.setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { props.setMenuOpen(false); props.submitSearch(); } }} size="small" /><ThemeSelect {...props} /><Button onClick={() => { props.setMenuOpen(false); props.onCart(); }} startIcon={<ShoppingBagOutlinedIcon />} sx={{ color: "inherit", justifyContent: "flex-start", mt: 1.5, px: 1.5 }}>Basket ({props.count})</Button></Box></Drawer>;
}

function PremiumDrawer(props: NavbarProps) {
  const theme = storefrontThemes[props.themeName];
  const dark = props.mode === "dark";
  return <Drawer anchor="left" onClose={() => props.setMenuOpen(false)} open={props.menuOpen}><Box sx={{ bgcolor: dark ? theme.darkPaper : "#fffaf8", color: dark ? theme.darkText : "#3f2631", minHeight: "100%", p: 3, width: 320 }}><Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between" }}><HeaderLogo onClick={() => props.go("/twc-ecommerce")} /><IconButton onClick={() => props.setMenuOpen(false)} aria-label="Close menu"><CloseRoundedIcon /></IconButton></Stack><Typography sx={{ color: dark ? theme.accent : "#8b4962", fontSize: 11, fontWeight: 900, letterSpacing: ".16em", mt: 6 }}>THE TWC EDIT</Typography><Stack spacing={1} sx={{ mt: 2 }}><Button onClick={() => props.go("/twc-ecommerce/shop")} sx={{ color: "inherit", justifyContent: "flex-start", fontFamily: 'Georgia, "Times New Roman", serif', fontSize: 24, fontWeight: 500, px: 0 }}>Shop</Button><Button onClick={() => props.goToSection("collections")} sx={{ color: "inherit", justifyContent: "flex-start", fontFamily: 'Georgia, "Times New Roman", serif', fontSize: 24, fontWeight: 500, px: 0 }}>Collections</Button><Button onClick={() => props.goToSection("journal")} sx={{ color: "inherit", justifyContent: "flex-start", fontFamily: 'Georgia, "Times New Roman", serif', fontSize: 24, fontWeight: 500, px: 0 }}>Journal</Button><Button onClick={() => props.goToSection("about")} sx={{ color: "inherit", justifyContent: "flex-start", fontFamily: 'Georgia, "Times New Roman", serif', fontSize: 24, fontWeight: 500, px: 0 }}>About</Button></Stack><Divider sx={{ my: 3 }} /><ThemeSelect {...props} /><Button onClick={() => { props.setMenuOpen(false); props.onCart(); }} startIcon={<ShoppingBagOutlinedIcon />} sx={{ color: "inherit", justifyContent: "flex-start", mt: 2, px: 0 }}>Basket ({props.count})</Button></Box></Drawer>;
}

function CorporateNavbar(props: NavbarProps) {
  return <><Box sx={{ bgcolor: "primary.main", color: "primary.contrastText", display: { xs: "none", md: "block" }, px: 3, py: .6 }}><Container maxWidth="xl"><Typography sx={{ fontSize: 11, fontWeight: 800, letterSpacing: ".08em" }}>TWC COMMERCE PLATFORM · OFFLINE PORTFOLIO DEMONSTRATION</Typography></Container></Box><AppBar position="sticky" elevation={0} sx={{ bgcolor: "background.paper", color: "text.primary", borderBottom: 1, borderColor: "divider" }}><Container maxWidth="xl"><Toolbar disableGutters sx={{ gap: 3, minHeight: 76 }}><IconButton onClick={() => props.setMenuOpen(true)} sx={{ display: { xs: "inline-flex", lg: "none" } }} aria-label="Open menu"><MenuRoundedIcon /></IconButton><HeaderLogo onClick={() => props.go("/twc-ecommerce")} /><Stack direction="row" spacing={1} sx={{ display: { xs: "none", lg: "flex" } }}><Button onClick={() => props.go("/twc-ecommerce/shop")} sx={{ color: "text.primary" }}>Products</Button><Button onClick={() => props.goToSection("benefits")} sx={{ color: "text.primary" }}>Solutions</Button><Button onClick={() => props.goToSection("collections")} sx={{ color: "text.primary" }}>Collections</Button><Button onClick={() => props.goToSection("about")} sx={{ color: "text.primary" }}>About</Button><Button onClick={() => props.goToSection("faq")} sx={{ color: "text.primary" }}>Support</Button></Stack><Box sx={{ flexGrow: 1 }} /><ThemeSelect {...props} /><ThemeModeAndCart {...props} /></Toolbar></Container></AppBar><CorporateDrawer {...props} /></>;
}

function CorporateDrawer(props: NavbarProps) {
  const theme = storefrontThemes[props.themeName];
  const dark = props.mode === "dark";
  return <Drawer anchor="left" onClose={() => props.setMenuOpen(false)} open={props.menuOpen}><Box sx={{ bgcolor: dark ? theme.darkPaper : theme.lightSurface, color: dark ? theme.darkText : "#173044", minHeight: "100%", p: { xs: 2.5, sm: 3 }, width: { xs: 300, sm: 350 } }}><Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between" }}><HeaderLogo onClick={() => props.go("/twc-ecommerce")} /><IconButton onClick={() => props.setMenuOpen(false)} aria-label="Close corporate menu"><CloseRoundedIcon /></IconButton></Stack><Box sx={{ bgcolor: dark ? theme.darkSurface : "#173044", color: "#fff", mt: 4, p: 2.5 }}><Typography sx={{ color: dark ? theme.accent : "#b9cddd", fontSize: 11, fontWeight: 900, letterSpacing: ".14em" }}>CORPORATE COMMERCE</Typography><Typography sx={{ fontSize: 20, fontWeight: 900, lineHeight: 1.2, mt: .75 }}>Clear paths to useful products.</Typography></Box><Stack spacing={.75} sx={{ mt: 3 }}><Button onClick={() => props.go("/twc-ecommerce")} sx={{ color: "inherit", justifyContent: "flex-start", fontWeight: 800 }}>Home</Button><Button onClick={() => props.go("/twc-ecommerce/shop")} sx={{ color: "inherit", justifyContent: "flex-start", fontWeight: 800 }}>Products</Button><Button onClick={() => props.goToSection("benefits")} sx={{ color: "inherit", justifyContent: "flex-start", fontWeight: 800 }}>Solutions</Button><Button onClick={() => props.goToSection("collections")} sx={{ color: "inherit", justifyContent: "flex-start", fontWeight: 800 }}>Collections</Button><Button onClick={() => props.goToSection("about")} sx={{ color: "inherit", justifyContent: "flex-start", fontWeight: 800 }}>About</Button><Button onClick={() => props.goToSection("faq")} sx={{ color: "inherit", justifyContent: "flex-start", fontWeight: 800 }}>Support</Button></Stack><Divider sx={{ my: 3 }} /><TextField fullWidth label="Search products" value={props.query} onChange={(event) => props.setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { props.setMenuOpen(false); props.submitSearch(); } }} size="small" /><ThemeSelect {...props} /><Button onClick={() => { props.setMenuOpen(false); props.onCart(); }} startIcon={<ShoppingBagOutlinedIcon />} sx={{ color: "inherit", justifyContent: "flex-start", mt: 1.5 }}>Basket ({props.count})</Button></Box></Drawer>;
}
