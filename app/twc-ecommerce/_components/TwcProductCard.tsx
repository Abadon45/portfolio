...
function ProductImage({ product, onOpen, height = 260, alt = product.name, surface = "action.hover" }: { product: StoreProduct; onOpen: () => void; height?: number | Record<string, number>; alt?: string; surface?: string }) {
  const isPremiumSurface = surface === "#f8efed";
  return <Box role="button" tabIndex={0} onClick={onOpen} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpen(); } }} aria-label={`View ${product.name}`} sx={{ bgcolor: surface, cursor: "pointer", height, overflow: "hidden", position: "relative", "&:focus-visible": { outline: 2, outlineColor: "primary.main", outlineOffset: -2 } }}><Box component="img" src={product.image} alt={alt} loading="lazy" sx={{ height: "100%", objectFit: "contain", p: isPremiumSurface ? { xs: 0.5, md: 0.75 } : { xs: 1.5, md: 2 }, transition: "transform .45s ease", width: "100%", "&:hover": { transform: "scale(1.05)" } }} /></Box>;
}

function PremiumCard({ product, added, onAdd, onOpen }: CardProps) {
  return <Card sx={{ bgcolor: "background.paper", border: 1, borderColor: "rgba(139,73,98,.14)", borderRadius: 2, boxShadow: "0 12px 30px rgba(72,33,48,.06)", display: "flex", flexDirection: "column", height: "100%", overflow: "hidden", p: { xs: 1, md: 1.25 }, transition: "box-shadow .3s, transform .3s", "&:hover": { boxShadow: "0 18px 42px rgba(72,33,48,.12)", transform: "translateY(-4px)" } }}><ProductImage height={{ xs: 190, sm: 205, lg: 195 }} onOpen={onOpen} product={product} surface="#f8efed" /><CardContent sx={{ display: "flex", flex: 1, flexDirection: "column", minHeight: { xs: 140, md: 150 }, p: { xs: 1, sm: 1.5, md: 1.25 } }}><Typography color="text.secondary" sx={{ fontSize: 9, fontWeight: 900, letterSpacing: ".12em", textTransform: "uppercase" }}>{product.category}</Typography><Typography onClick={onOpen} component="h3" sx={{ cursor: "pointer", display: "-webkit-box", fontFamily: 'Georgia, "Times New Roman", serif', fontSize: { xs: 15, sm: 16, md: 16 }, fontWeight: 500, lineHeight: 1.2, mt: .5, minHeight: { xs: 36, md: 38 }, overflow: "hidden", WebkitBoxOrient: "vertical", WebkitLineClamp: 2 }}>{product.name}</Typography><Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between", mt: "auto", minHeight: 32, pt: 1 }}><Typography sx={{ fontSize: { xs: 14, md: 15 }, fontWeight: 900 }}>{money(product.price)}</Typography><Button disabled={added} onClick={onAdd} endIcon={<ArrowForwardRoundedIcon />} size="small" sx={{ color: "primary.main", minWidth: 0, px: .5 }}>{added ? "Added" : "Add"}</Button></Stack></CardContent></Card>;
}
