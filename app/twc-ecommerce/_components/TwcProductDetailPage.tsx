...
function EditorialProductDetail(props: DetailProps) {
  const router = useRouter();
  return <Container maxWidth="lg" sx={{ py: { xs: 3, md: 7 } }}><Button onClick={() => router.push("/twc-ecommerce/shop")} startIcon={<ArrowBackRoundedIcon />} sx={{ color: "text.secondary", mb: 3 }}>Back to the edit</Button><Box sx={{ display: "grid", gap: { xs: 4, md: 9 }, gridTemplateColumns: { xs: "1fr", md: "1.15fr .85fr" } }}><ProductGallery editorial product={props.product} /><Box sx={{ alignSelf: "center" }}><PurchasePanel {...props} eyebrow="THE EDIT" /><Typography color="text.secondary" sx={{ borderLeft: 2, borderColor: "primary.main", lineHeight: 1.8, mt: 4, pl: 2 }}>A considered addition to a slower, more intentional daily ritual.</Typography></Box></Box><Paper sx={{ bgcolor: "transparent", borderTop: 1, borderColor: "divider", borderRadius: 0, mt: { xs: 7, md: 12 }, pt: 4 }}><Typography component="h2" sx={{ fontFamily: 'Georgia, "Times New Roman", serif', fontSize: { xs: 30, md: 44 }, fontWeight: 500 }}>The story behind the choice.</Typography><Typography color="text.secondary" sx={{ lineHeight: 1.9, maxWidth: 720, mt: 2, whiteSpace: "pre-line" }}>{props.product.details || props.product.description}</Typography></Paper><RelatedProducts product={props.product} onAdd={props.onRelatedAdd} /></Container>;
}
